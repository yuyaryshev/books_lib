...
export class BaseDomain<RowT extends RowWithIdT, RowDefaultFieldsT extends keyof RowT, ObjT, ObjSerializedT, TableT extends BaseTable<RowT, RowDefaultFieldsT>> {
  ...
  protected async rowToObj(row: RowT, extend: ExtendedFieldCategoriesObj): Promise<ObjT> {
    const r = {
      type: this.domainDescriptor.name,
      ...(row as any)
    };
    for (let k in r) {
  ...
  async upsert(obj: {
    ...
    return this.domainDescriptor.table.knexTable().where(({
      id
    } as any)).update(columnsToUpdate);
  }
  delete(id: BaseId) {
