...
export class QueueTable<RowT extends object> {
  ...
  constructor(knex: Knex, tableDescriptor: TableDescriptor, columnNamesStr: string) {
    ...
    this.knex = knex;
    this.columnNames = columnNamesStr.split(" ").map(s => s.trim()).filter(s => s.length);
  }
  knexTable() {
  ...
  async upsert(obj: RowT) {
    await this.knexTable().insert((obj as any)).onConflict().ignore();
  }
  ...
  async copyFrom(sourceTableName: string, doDistinct: boolean) {
    ...
    return await this.knexTable().insert((query as any)).onConflict().ignore();
  }
  async createTable() {
