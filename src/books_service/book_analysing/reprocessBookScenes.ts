...
export async function reprocessBookScenes(env: ServiceApiEnv, bookIds0?: BookId[] | undefined) {
  const bookIds = bookIds0 !== undefined ? bookIds0 : (await env.tables.book_scene_queue.select().limit(BOOK_SCENES_PROCESSING_BATCH)).map(row => row.book_id);
  for (let book_id of bookIds) {
    const sceneRows = await env.tables.book_scenes.select().where({
      book_id
    });
    for (let sceneRow of sceneRows) {
      ...
      const traitsRow = {
        ...extractedColumns,
        json: JSON.stringify(processedScene),
        id: book_scene_id,
        book_id
      };
      await env.tables.book_scene_traits.upsertById(traitsRow);
      await env.tables.book_scene_queue.remove({
        book_id,
        book_scene_id
      });
    }
